...
from database.globals import GEO_REPLICATION_DB, DB_HOST
...
from geohelp.helpers import haversine, select_best_guess_centroid
...
def compute_geo_info(probes, serialized_file):
    ...
    print(f"Anchors in {len(countries_anchors)} countries")
    print(f"Anchors in {len(ases_anchors)} ASes")
    print(f"Probes in {len(countries_probes)} countries")
    print(f"Probes in {len(ases_probes)} ASes")
...
def compute_error(dst, vp_coordinates_per_ip, rtt_per_src):
    error = None
    circles = []
    guessed_geolocation_circles = select_best_guess_centroid(dst, vp_coordinates_per_ip, rtt_per_src)

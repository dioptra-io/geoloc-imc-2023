...
from analysis.analysis import compute_geo_info
...
def plot_error_threshold(threshold_distances, results_file, country_per_ip, vps_per_country):
    ...
    if is_compute_error_continent:
        ...
        anchors_second = set(
            load_json("resources/replicability/anchors_ip_lst.json"))
...
def plot_fixed_subset(fixed_set_probes_one_per_city_file, fixed_set_probes_one_per_city_asn_file, min_rtt_per_dst_file):
    ...
    fixed_set_probes_results_per_city = load_json(
        fixed_set_probes_one_per_city_file)
    fixed_set_probes_results_per_city_asn = load_json(
        fixed_set_probes_one_per_city_asn_file)
...
if __name__ == "__main__":
    ...
    if is_plot_error:
        results_file = [anchor_to_anchors_results_file,
                        probes_to_anchors_results_file]
        ...
        plot_error_threshold(threshold_distances, results_file,
                             country_per_vp, vps_per_country)
    ...
    if is_plot_fixed_set:
        plot_fixed_subset(fixed_set_probes_one_per_city_file,
                          fixed_set_probes_one_per_city_asn_file, min_rtt_per_dst_file)

    if is_plot_round_based_algorithm:
        plot_round_based_algorithm(
            round_based_algorithm_file, probes_to_anchors_results_file)

    if is_plot_min_rtt:
        with open(FINAL_ANALYSABLE_FILE, 'r') as json_file:

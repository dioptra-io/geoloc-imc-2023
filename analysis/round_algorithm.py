...
from default import PROBES_TO_ANCHORS_PING_TABLE, ANCHORS_FILE, PROBES_AND_ANCHORS_FILE, REMOVED_PROBES_FILE
...
def evaluate():
    ...
    if is_compute_round_algorithm:
        ...
        if not os.path.exists(GREEDY_PROBES_FILE):
            greedy_probes = greedy_selection_probes(
                vp_distance_matrix, removed_probes, country_per_vp, limit=1000)
        ...
        if os.path.exists(round_based_algorithm_file):
            ...
            error_cdf_per_tier1_vps = {
                int(x): error_cdf_per_tier1_vps[x] for x in error_cdf_per_tier1_vps}
